...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as Geminis, {
    get(_, property) {
      return createFunctionalProxy([property], alovaInstance, configMap);
    }
  });
  // define global variable `Apis`
  (globalThis as any).Geminis = Apis;
  return Apis;
};
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof Geminis
  ? Url extends keyof Geminis[Tag]
    ? Geminis[Tag][Url] extends (...args: any) => any
      ? Parameters<Geminis[Tag][Url]>
      : any
    : any
  : any;
